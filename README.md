...
The container needs access to the `/dev/kmsg` device, which can be achieved by running the container with the appropriate permissions. For example, you can use the following command to run the container with access to `/dev/kmsg`:
